...
try {
    ...
    /* ===============================
       BASIC VALIDATION
    ================================ */
    if (empty($customerName) || empty($contactNo) || empty($email) || empty($address) || empty($subject) || empty($message)) 
    {
    ...
    $rew = $conn->prepare(
        ...
        (Customer_Id, Subject, Message, Created_Date)
        VALUES (?, ?, ?, NOW())"
    );
    $rew->bind_param(
        "sss",
